...
def load():
    ...
    puzzle = puzzle[:puzzle.find('var logic = null;')]
    answer = pop.answer
...
class view(threading.Thread):
    ...
    def __init__(self):
        ...
        self.main_window.set_default_size(
            gtk.gdk.screen_width() - 100, gtk.gdk.screen_height() - 100)
...
# loader
        ...
        self.loader.set_markup(
            '<span color="green" size="16000">Loading....</span>')
        self.loader_cont.pack_start(self.loader, True)
        self.loader_cont1 = gtk.VBox()
        self.loader1 = gtk.Label()
        self.loader1.set_markup(
            '<span color="green" size="16000">Loading....</span>')
        self.loader_cont1.pack_start(self.loader1, True)
# pack three boxes in window into main Horizontal box
        ...
        self.vbox_two.set_size_request(
            self.wid_two, gtk.gdk.screen_height() - 100)
        self.vbox_one.set_size_request(
            self.wid_one, gtk.gdk.screen_height() - 100)
    ...
    def sidebar(self):
        ...
        self.letter_equation.modify_bg(
            gtk.STATE_NORMAL, self.letter_equation.get_colormap().alloc_color('black'))
        ...
        self.crypto.modify_bg(
            gtk.STATE_NORMAL, self.crypto.get_colormap().alloc_color('light gray'))
        self.vbox_one.pack_start(self.crypto, False)
        self.science = gtk.Button('Science')
        self.science.set_size_request(200, 33)
        self.science.modify_bg(
            gtk.STATE_NORMAL, self.science.get_colormap().alloc_color('black'))
        self.vbox_one.pack_start(self.science, False)
        self.back = gtk.Button('BACK')
        self.back.set_size_request(200, 33)
        self.back.modify_bg(
            gtk.STATE_NORMAL, self.back.get_colormap().alloc_color('red'))
    ...
    def question(self):
        ...
        self.text_view1.modify_font(
            pango.FontDescription('monospace regular 13'))
        ...
        self.text_buffer1.set_text(puzzle)
        self.view1.set_size_request(
            gtk.gdk.screen_width() - 210, gtk.gdk.screen_height())
    ...
    def ans(self, etc):
        ...
        self.view1.set_size_request(
            gtk.gdk.screen_width() - 210, gtk.gdk.screen_height() / 2)
        self.view2.set_size_request(
            gtk.gdk.screen_width() - 210, gtk.gdk.screen_height() / 2)
    ...
    def details(self):
        ...
        self.cat_lab = gtk.Label()
        self.cat_lab.modify_fg(
            gtk.STATE_NORMAL, self.cat_lab.get_colormap().alloc_color('dark blue'))
        self.diff_lab = gtk.Label('{ ' + str(difficulty) + ' }')
        self.cat_lab.set_markup('<span size="10000">' + category + '</span>')
        ...
        self.cat_but.modify_bg(
            gtk.STATE_NORMAL, self.cat_but.get_colormap().alloc_color('light gray'))
        ...
        self.ans_but.modify_bg(
            gtk.STATE_NORMAL, self.ans_but.get_colormap().alloc_color('light gray'))
        ...
        self.rand_but.modify_bg(
            gtk.STATE_NORMAL, self.rand_but.get_colormap().alloc_color('dark gray'))
        ...
        self.same_but.modify_bg(
            gtk.STATE_NORMAL, self.same_but.get_colormap().alloc_color('dark gray'))
        ...
        self.hint_but.modify_bg(
            gtk.STATE_NORMAL, self.hint_but.get_colormap().alloc_color('black'))
        ...
        self.vbox_one.pack_start(self.loader_cont1, False)
    ...
    def update(self):
        ...
        if hint == '':
            ...
            self.hint_hori.hide()
        if difficulty < 0.90:
            self.diff_lab.modify_fg(
                gtk.STATE_NORMAL, self.diff_lab.get_colormap().alloc_color('green'))
        elif difficulty < 1.70:
            self.diff_lab.modify_fg(
                gtk.STATE_NORMAL, self.diff_lab.get_colormap().alloc_color('orange'))
        else:
            self.diff_lab.modify_fg(
                gtk.STATE_NORMAL, self.diff_lab.get_colormap().alloc_color('red'))

    def next_same(self, etc):
        store = {'LOGIC': 'Logic', 'LOGIC-GRID': 'Logic-Grid', 'CRYPTOGRAPHY': 'Cryptography', 'SCIENCE': 'Science', 'LETTER-EQUATIONS': 'Letter-Equations', 'RIDDLE': 'Riddle', 'TRICK': 'Trick',
                 'RIBUS': 'Ribus', 'LANGUAGE': 'Language', 'MYSTERY': 'Mystery', 'SITUATION': 'Situation', 'TRIVIA': 'Trivia', 'PROBABILITY': 'Probability', 'SERIES': 'Series', 'MATH': 'Math'}
        self.opn('', self.default_add +
                 '/teaser.php?similar=' + store[category])

    def action(self):
        self.rand_but.connect('clicked', self.opn,
                              self.default_add + '/teaser.php?rand=1')
        self.same_but.connect('clicked', self.next_same)
        self.crypto.connect(
            'clicked', self.opn, self.default_add + '/teaser.php?similar=Cryptography')
        self.science.connect('clicked', self.opn,
                             self.default_add + '/teaser.php?similar=Science')
        self.logic.connect('clicked', self.opn,
                           self.default_add + '/teaser.php?similar=Logic')
        self.letter_equation.connect(
            'clicked', self.opn, self.default_add + '/teaser.php?similar=Letter-Equations')
        self.rebus.connect('clicked', self.opn,
                           self.default_add + '/teaser.php?similar=Rebus')
        self.riddle.connect('clicked', self.opn,
                            self.default_add + '/teaser.php?similar=Riddle')
        self.trick.connect('clicked', self.opn,
                           self.default_add + '/teaser.php?similar=Trick')
        self.language.connect(
            'clicked', self.opn, self.default_add + '/teaser.php?similar=Language')
        self.logic_grid.connect(
            'clicked', self.opn, self.default_add + '/teaser.php?similar=Logic-Grid')
        self.math.connect('clicked', self.opn,
                          self.default_add + '/teaser.php?similar=Math')
        self.mystery.connect('clicked', self.opn,
                             self.default_add + '/teaser.php?similar=Mystery')
        self.situation.connect(
            'clicked', self.opn, self.default_add + '/teaser.php?similar=Situation')
        self.trivia.connect('clicked', self.opn,
                            self.default_add + '/teaser.php?similar=Trivia')
        self.probability.connect(
            'clicked', self.opn, self.default_add + '/teaser.php?similar=Probability')
        self.series.connect('clicked', self.opn,
                            self.default_add + '/teaser.php?similar=Series')
